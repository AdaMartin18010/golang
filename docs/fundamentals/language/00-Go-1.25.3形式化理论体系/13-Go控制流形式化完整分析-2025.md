...
- [更新可达定义](#更新可达定义)
- [计算OUT[block]](#计算outblock)
- [如果改变,更新后继](#如果改变更新后继)
- [交集操作](#交集操作)
  - [第四部分: Go特定控制流](#第四部分-go特定控制流)
  - [第五部分: 抽象解释](#第五部分-抽象解释)
- [检查true分支可行性](#检查true分支可行性)
- [检查false分支可行性](#检查false分支可行性)
- [路径探索完成](#路径探索完成)
- [符号执行找到所有路径](#符号执行找到所有路径)
- [求解路径约束](#求解路径约束)
- [迭代计算](#迭代计算)
- [精化 (Refinement)](#精化-refinement)
- [true分支: x ∈ [-∞, c-1]](#true分支-x-c-1)
- [false分支: x ∈ [c, +∞]](#false分支-x-c)
- [检查下界](#检查下界)
- [检查上界](#检查上界)
  - [第六部分: 编译器优化](#第六部分-编译器优化)
- [从entry开始DFS](#从entry开始dfs)
- [删除不可达块](#删除不可达块)
- [活跃变量分析](#活跃变量分析)
- [x不活跃,删除赋值](#x不活跃删除赋值)
- [更新活跃集](#更新活跃集)
...
- [复用已有临时变量](#复用已有临时变量)
- [创建新临时变量](#创建新临时变量)
- [在支配点插入: temp = e](#在支配点插入-temp-e)
- [新表达式](#新表达式)
- [识别循环不变计算](#识别循环不变计算)
- [检查支配关系](#检查支配关系)
- [instr定义的变量必须支配所有使用点](#instr定义的变量必须支配所有使用点)
- [移到preheader](#移到preheader)
- [创建factor-1份循环体副本](#创建factor-1份循环体副本)
- [更新循环计数器](#更新循环计数器)
- [条件: 相同迭代空间,无数据依赖](#条件-相同迭代空间无数据依赖)
- [合并循环体](#合并循环体)
- [将循环分成两个独立循环](#将循环分成两个独立循环)
- [检查向量化条件](#检查向量化条件)
- [转换标量操作为向量操作](#转换标量操作为向量操作)
- [更新循环计数](#更新循环计数)
  - [🎯 总结](#总结)
