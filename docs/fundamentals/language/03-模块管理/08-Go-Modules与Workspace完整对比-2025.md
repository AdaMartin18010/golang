...
- [Go Modules与Workspace完整对比（2025最新）](#go-modules与workspace完整对比2025最新)
  - [1. 📊 核心概念对比](#1-核心概念对比)
  - [2. 🔄 工作流程对比](#2-工作流程对比)
- [创建新项目](#创建新项目)
- [初始化模块](#初始化模块)
...
- [项目结构](#项目结构)
- [开发任意模块](#开发任意模块)
- [方式1: 代码中import后自动添加](#方式1-代码中import后自动添加)
- [main.go:](#maingo)
- [import "github.com/gin-gonic/gin"](#import-githubcomgin-gonicgin)
- [✅ 自动添加到 go.mod](#自动添加到-gomod)
- [✅ 下载依赖](#下载依赖)
- [✅ 更新 go.sum](#更新-gosum)
- [方式2: 手动添加](#方式2-手动添加)
- [方式3: 编辑go.mod后](#方式3-编辑gomod后)
- [service-a 需要使用 shared](#service-a-需要使用-shared)
- [service-a/main.go:](#service-amaingo)
- [import "example.com/shared"](#import-examplecomshared)
- [无需任何操作！](#无需任何操作)
- [✅ workspace自动识别本地模块](#workspace自动识别本地模块)
- [✅ 修改shared立即生效](#修改shared立即生效)
- [✅ 无需replace指令](#无需replace指令)
- [同步所有模块依赖](#同步所有模块依赖)
- [更新单个依赖](#更新单个依赖)
- [更新所有依赖](#更新所有依赖)
- [更新到特定版本](#更新到特定版本)
- [清理未使用依赖](#清理未使用依赖)
- [查看可用更新](#查看可用更新)
- [workspace不管理依赖版本](#workspace不管理依赖版本)
- [需要在各模块中分别更新](#需要在各模块中分别更新)
- [方式1: 逐个更新](#方式1-逐个更新)
- [方式2: 使用脚本批量更新](#方式2-使用脚本批量更新)
- [同步workspace](#同步workspace)
- [需要修改第三方库调试](#需要修改第三方库调试)
- [1. Fork并clone库](#1-fork并clone库)
- [2. 修改go.mod添加replace](#2-修改gomod添加replace)
- [go.mod:](#gomod)
- [3. 测试](#3-测试)
- [4. 调试完成后删除replace](#4-调试完成后删除replace)
- [⚠️ 容易忘记删除](#️-容易忘记删除)
- [无需修改go.mod](#无需修改gomod)
- [1. 创建workspace](#1-创建workspace)
- [2. 直接测试](#2-直接测试)
- [3. 调试完成删除go.work](#3-调试完成删除gowork)
- [✅ go.mod保持干净](#gomod保持干净)
  - [3. 🎯 使用场景选择](#3-使用场景选择)
  - [4. ⚡ 性能与效率](#4-性能与效率)
- [首次构建（冷缓存）](#首次构建冷缓存)
- [real: 45.2s](#real-452s)
- [增量构建（修改1个文件）](#增量构建修改1个文件)
- [real: 2.1s](#real-21s)
- [清理缓存后](#清理缓存后)
- [real: 52.8s (重新下载依赖)](#real-528s-重新下载依赖)
- [首次构建（冷缓存）](#首次构建冷缓存)
- [real: 43.8s (略快，使用本地模块)](#real-438s-略快使用本地模块)
- [增量构建（修改1个文件）](#增量构建修改1个文件)
- [real: 2.0s (相当)](#real-20s-相当)
- [跨模块修改（修改shared模块）](#跨模块修改修改shared模块)
- [real: 8.5s (重建依赖模块)](#real-85s-重建依赖模块)
- [监控Go工具链内存使用](#监控go工具链内存使用)
- [Go Modules模式](#go-modules模式)
- [峰值内存: ~450MB](#峰值内存-450mb)
- [Workspace模式（10个模块）](#workspace模式10个模块)
- [峰值内存: ~620MB (+38%)](#峰值内存-620mb-38)
- [Go Modules](#go-modules)
- [依赖缓存: $GOMODCACHE (~2GB for typical project)](#依赖缓存-gomodcache-2gb-for-typical-project)
- [2.1G](#21g)
- [Workspace](#workspace)
- [缓存相同，但本地模块占空间](#缓存相同但本地模块占空间)
- [850M (所有模块源码)](#850m-所有模块源码)
- [总计: 2.1G + 0.85G = 2.95G](#总计-21g-085g-295g)
  - [5. 📚 相关资源](#5-相关资源)
- [推荐的项目结构](#推荐的项目结构)
- [小型项目/库](#小型项目库)
- [中大型项目](#中大型项目)
