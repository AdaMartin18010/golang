...
- [Go Modules 简介](#go-modules-简介)
  - [1. 📚 理论分析](#1-理论分析)
- [GOPATH 模式无法指定版本](#gopath-模式无法指定版本)
- [必须在 GOPATH/src 下创建项目](#必须在-gopathsrc-下创建项目)
  - [2. 💻 代码示例](#2-代码示例)
- [1. 创建项目目录](#1-创建项目目录)
- [2. 初始化模块](#2-初始化模块)
- [3. 查看 go.mod](#3-查看-gomod)
- [直接运行](#直接运行)
- [或构建后运行](#或构建后运行)
- [方式1: go mod tidy (推荐)](#方式1-go-mod-tidy-推荐)
- [方式2: go get](#方式2-go-get)
- [方式3: go build/run (自动下载)](#方式3-go-buildrun-自动下载)
- [使用最新版本](#使用最新版本)
- [使用特定版本](#使用特定版本)
- [使用特定 commit](#使用特定-commit)
- [使用分支](#使用分支)
- [更新所有依赖到最新版本](#更新所有依赖到最新版本)
- [更新指定包](#更新指定包)
- [更新 patch 版本 (1.9.1 -> 1.9.2)](#更新-patch-版本-191-192)
  - [3. 🔧 实践应用](#3-实践应用)
- [1. 进入项目目录 (可以在任意位置)](#1-进入项目目录-可以在任意位置)
- [2. 初始化模块](#2-初始化模块)
- [3. 自动导入依赖](#3-自动导入依赖)
- [4. 验证构建](#4-验证构建)
- [5. 提交 go.mod 和 go.sum](#5-提交-gomod-和-gosum)
- [1. 创建 workspace](#1-创建-workspace)
- [2. 查看 go.work](#2-查看-gowork)
  - [4. 📊 对比分析](#4-对比分析)
  - [5. 🎯 最佳实践](#5-最佳实践)
- [添加缺失的依赖，删除未使用的依赖](#添加缺失的依赖删除未使用的依赖)
- [推荐](#推荐)
- [避免](#避免)
- [每月检查一次](#每月检查一次)
- [go.sum 由 Go 工具自动维护](#gosum-由-go-工具自动维护)
- [.gitignore 中不要添加 go.sum](#gitignore-中不要添加-gosum)
  - [6. ⚠️ 常见陷阱](#6-️-常见陷阱)
- [使用 replace 统一版本](#使用-replace-统一版本)
- [配置私有模块](#配置私有模块)
- [配置 Git 凭证](#配置-git-凭证)
- [使用国内镜像](#使用国内镜像)
  - [7. 🔍 常见问题](#7-常见问题)
- [查看当前模式](#查看当前模式)
- [on: 强制使用 Go Modules](#on-强制使用-go-modules)
- [off: 强制使用 GOPATH](#off-强制使用-gopath)
- [auto: 自动判断 (不推荐)](#auto-自动判断-不推荐)
- [列出所有依赖](#列出所有依赖)
- [列出直接依赖](#列出直接依赖)
- [查看依赖树](#查看依赖树)
- [降级到 v1.8.0](#降级到-v180)
