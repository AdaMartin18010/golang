...
- [项目结构](#项目结构)
- [引用外部模块](#引用外部模块)
- [→ 从远程仓库或缓存获取](#从远程仓库或缓存获取)
- [初始化](#初始化)
- [修改库代码](#修改库代码)
- [立即测试示例](#立即测试示例)
- [无需发布即可验证](#无需发布即可验证)
- [方式1: 手动添加所有Go模块](#方式1-手动添加所有go模块)
- [方式2: 使用脚本批量添加](#方式2-使用脚本批量添加)
- [1. Clone需要调试的库](#1-clone需要调试的库)
- [2. 创建workspace](#2-创建workspace)
- [3. 开始调试](#3-开始调试)
- [go.mod保持不变](#gomod保持不变)
  - [5. 🎯 最佳实践](#5-最佳实践)
- [.gitignore](#gitignore)
- [原因：](#原因)
- [- workspace配置是开发者本地环境](#workspace配置是开发者本地环境)
- [- 不应影响其他开发者](#不应影响其他开发者)
- [- CI/CD应使用go.mod构建](#cicd应使用gomod构建)
- [go.work.example - 提供给团队参考](#goworkexample-提供给团队参考)
- [团队成员复制并自定义](#团队成员复制并自定义)
- [cp go.work.example go.work](#cp-goworkexample-gowork)
- [README.md](#readmemd)
  - [本地开发设置](#本地开发设置)
- [1. 复制workspace模板](#1-复制workspace模板)
- [2. 根据需要添加模块](#2-根据需要添加模块)
- [3. 同步依赖](#3-同步依赖)
- [初始化workspace](#初始化workspace)
- [添加模块到workspace](#添加模块到workspace)
- [编辑go.work（类似go mod edit）](#编辑gowork类似go-mod-edit)
- [同步workspace模块依赖](#同步workspace模块依赖)
- [查看workspace配置](#查看workspace配置)
- [临时禁用（单次命令）](#临时禁用单次命令)
- [环境变量禁用（当前会话）](#环境变量禁用当前会话)
- [强制使用特定workspace文件](#强制使用特定workspace文件)
- [.github/workflows/ci.yml](#githubworkflowsciyml)
- [Dockerfile](#dockerfile)
- [只复制go.mod和go.sum，不包含go.work](#只复制gomod和gosum不包含gowork)
- [确保不使用workspace](#确保不使用workspace)
- [使用go work sync统一版本](#使用go-work-sync统一版本)
- [检查不一致的依赖](#检查不一致的依赖)
- [查看当前使用的workspace](#查看当前使用的workspace)
- [查看workspace包含的模块](#查看workspace包含的模块)
- [查看所有依赖](#查看所有依赖)
- [验证模块路径](#验证模块路径)
- [错误](#错误)
- [解决](#解决)
- [1. 检查go.work中的use路径是否正确](#1-检查gowork中的use路径是否正确)
- [2. 确认模块目录包含go.mod](#2-确认模块目录包含gomod)
- [3. 运行 go work sync](#3-运行-go-work-sync)
- [解决步骤](#解决步骤)
- [1. 查看冲突详情](#1-查看冲突详情)
- [2. 使用replace统一版本](#2-使用replace统一版本)
- [go.work:](#gowork)
- [3. 同步所有模块](#3-同步所有模块)
- [清理缓存](#清理缓存)
- [重新下载依赖](#重新下载依赖)
- [验证workspace配置](#验证workspace配置)
  - [6. 📚 相关资源](#6-相关资源)
  - [📝 总结](#总结)
