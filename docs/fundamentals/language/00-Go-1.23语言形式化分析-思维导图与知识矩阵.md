...
**创建日期**: 2025年10月24日  
**Go版本**: Go 1.23  
**理论基础**: 形式语言理论 + 操作语义 + 类型理论 + CSP并发模型  
**文档类型**: 可视化分析 + 知识体系 + 属性关系
...
| Channel发送<br>`ch <- v` | `Send(ch, v)` | `⟨ch <- v, σ⟩ →`<br>`⟨skip, σ⟩`<br>（同步）| 阻塞直到接收 | CSP: `ch!v → P` | `Γ ⊢ ch : chan T, Γ ⊢ v : T`<br>`──────────────────────`<br>`Γ ⊢ ch <- v : ()` |
| Channel接收<br>`v := <-ch` | `Recv(ch)` | `⟨v := <-ch, σ⟩ →`<br>`⟨skip, σ[v↦val]⟩`<br>（同步）| 阻塞直到发送 | CSP: `ch?v → P` | `Γ ⊢ ch : chan T`<br>`────────────────`<br>`Γ ⊢ v := <-ch : T` |
...
| `chan T` | `nil` | ✅ | ✅ | `<-` (发送/接收), `close`, `len`, `cap` | 8 bytes (指针) |
