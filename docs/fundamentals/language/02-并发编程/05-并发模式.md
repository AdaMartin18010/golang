...
```go
func workerPool(jobs <-Channel int, results Channel<- int, numWorkers int) {
...
```go
func fanOut(ctx Context.Context, in <-Channel int, n int) []<-Channel int {
...
func worker(ctx Context.Context, in <-Channel int) <-Channel int {
    out := make(Channel int)
    go func() {
        defer close(out)
        for {
...
func doWorkWithTimeout(timeout time.Duration) (string, error) {
    resultCh := make(Channel string, 1)
...
func NewRateLimiter(rate int) *RateLimiter {
    ...
    rl := &RateLimiter{
        rate:   rate,
        bucket: make(Channel struct{}, rate),
        ctx:    ctx,
        cancel: cancel,
    }
...
```go
// ✅ 推荐：避免阻塞
results := make(Channel Result, numWorkers)

// ❌ 不推荐：可能导致goroutine泄漏
results := make(Channel Result)
```
...
func BenchmarkFanOut4(b *testing.B) {
    b.ResetTimer()
    for i := 0; i < b.N; i++ {
        in := make(Channel int, 100)
        out := make(Channel int, 100)
