...
```text
Process ::= Goroutine
Channel ::= make(Channel T [, capacity])
Send    ::= ch <- value
Receive ::= value := <-ch
Select  ::= select { case ... }
```
...
2. **Channel（Channel）**：进程间通信媒介，类型安全
...
```go
func workerPool(tasks <-Channel Task, results Channel<- Result, numWorkers int) {
    var wg sync.WaitGroup
...
// 阶段3：过滤
func filter(in <-Channel int, f func(int) bool) <-Channel int {
    out := make(Channel int)
    go func() {
        for n := range in {
            if f(n) {
