# 框架最佳实践指南

> **版本**: v1.0
> **日期**: 2025-01-XX
> **目标**: 提供框架使用的最佳实践和设计模式

---

## 📋 目录

- [框架最佳实践指南](#框架最佳实践指南)
  - [📋 目录](#-目录)
  - [1. 架构设计原则](#1-架构设计原则)
    - [1.1 依赖方向](#11-依赖方向)
    - [1.2 接口定义位置](#12-接口定义位置)
  - [2. 领域层设计](#2-领域层设计)
    - [2.1 实体设计](#21-实体设计)
    - [2.2 值对象设计](#22-值对象设计)
    - [2.3 领域服务](#23-领域服务)
  - [3. 应用层设计](#3-应用层设计)
    - [3.1 应用服务职责](#31-应用服务职责)
    - [3.2 DTO 设计](#32-dto-设计)
  - [4. 基础设施层设计](#4-基础设施层设计)
    - [4.1 仓储实现](#41-仓储实现)
    - [4.2 配置管理](#42-配置管理)
  - [5. 接口层设计](#5-接口层设计)
    - [5.1 HTTP Handler](#51-http-handler)
    - [5.2 中间件使用](#52-中间件使用)
  - [6. 错误处理](#6-错误处理)
    - [6.1 错误分类](#61-错误分类)
    - [6.2 错误包装](#62-错误包装)
  - [7. 日志记录](#7-日志记录)
    - [7.1 结构化日志](#71-结构化日志)
    - [7.2 日志级别](#72-日志级别)
  - [8. 测试策略](#8-测试策略)
    - [8.1 单元测试](#81-单元测试)
    - [8.2 集成测试](#82-集成测试)
  - [9. 性能优化](#9-性能优化)
    - [9.1 数据库查询](#91-数据库查询)
    - [9.2 缓存使用](#92-缓存使用)
  - [10. 安全实践](#10-安全实践)
    - [10.1 输入验证](#101-输入验证)
    - [10.2 认证授权](#102-认证授权)
  - [📚 相关文档](#-相关文档)

---

## 1. 架构设计原则

### 1.1 依赖方向

**原则**: 依赖应该向内指向领域层

```text
Interfaces → Application → Domain ← Infrastructure
```

**正确示例**:

```go
// ✅ 正确：接口层依赖应用层
type UserHandler struct {
    userService *application.UserService  // 依赖应用层
}

// ❌ 错误：接口层直接依赖基础设施层
type UserHandler struct {
    db *sql.DB  // 不应该直接依赖数据库
}
```

### 1.2 接口定义位置

**原则**: 接口定义在领域层，实现在基础设施层

```go
// ✅ 正确：接口在领域层
// internal/domain/user/repository.go
type Repository interface {
    FindByID(ctx context.Context, id string) (*User, error)
}

// ✅ 正确：实现在基础设施层
// internal/infrastructure/database/user_repository.go
type userRepository struct {
    client *ent.Client
}

func (r *userRepository) FindByID(ctx context.Context, id string) (*User, error) {
    // 实现
}
```

---

## 2. 领域层设计

### 2.1 实体设计

**原则**: 实体应该包含业务逻辑，而不是简单的数据容器

```go
// ✅ 正确：实体包含业务逻辑
type User struct {
    ID    string
    Email string
    Name  string
}

func (u *User) UpdateEmail(newEmail string) error {
    if !isValidEmail(newEmail) {
        return ErrInvalidEmail
    }
    u.Email = newEmail
    return nil
}

// ❌ 错误：实体只是数据容器
type User struct {
    ID    string
    Email string
    Name  string
}
// 业务逻辑在外部
```

### 2.2 值对象设计

**原则**: 使用值对象表示没有唯一标识的概念

```go
// ✅ 正确：使用值对象
type Email struct {
    value string
}

func NewEmail(value string) (Email, error) {
    if !isValidEmail(value) {
        return Email{}, ErrInvalidEmail
    }
    return Email{value: value}, nil
}

func (e Email) String() string {
    return e.value
}
```

### 2.3 领域服务

**原则**: 当业务逻辑不属于任何实体时，使用领域服务

```go
// ✅ 正确：使用领域服务处理跨实体的业务逻辑
type TransferService struct {
    accountRepo AccountRepository
}

func (s *TransferService) Transfer(from, to AccountID, amount Money) error {
    // 跨实体的业务逻辑
}
```

---

## 3. 应用层设计

### 3.1 应用服务职责

**原则**: 应用服务负责用例编排，不包含业务逻辑

```go
// ✅ 正确：应用服务编排用例
func (s *UserService) CreateUser(ctx context.Context, req CreateUserRequest) (*UserResponse, error) {
    // 1. 验证输入
    if err := s.validateRequest(req); err != nil {
        return nil, err
    }

    // 2. 调用领域层
    user, err := domain.NewUser(req.Email, req.Name)
    if err != nil {
        return nil, err
    }

    // 3. 持久化
    if err := s.userRepo.Save(ctx, user); err != nil {
        return nil, err
    }

    // 4. 发布事件
    s.eventBus.Publish(domain.NewUserCreatedEvent(user))

    // 5. 返回 DTO
    return toUserResponse(user), nil
}
```

### 3.2 DTO 设计

**原则**: 使用 DTO 在层之间传输数据

```go
// ✅ 正确：使用 DTO
type CreateUserRequest struct {
    Email string `json:"email" validate:"required,email"`
    Name  string `json:"name" validate:"required,min=2,max=50"`
}

type UserResponse struct {
    ID    string `json:"id"`
    Email string `json:"email"`
    Name  string `json:"name"`
}
```

---

## 4. 基础设施层设计

### 4.1 仓储实现

**原则**: 仓储实现应该隐藏技术细节

```go
// ✅ 正确：隐藏技术细节
func (r *userRepository) FindByID(ctx context.Context, id string) (*domain.User, error) {
    entUser, err := r.client.User.Get(ctx, id)
    if err != nil {
        if ent.IsNotFound(err) {
            return nil, domain.ErrUserNotFound
        }
        return nil, err
    }
    return toDomainUser(entUser), nil
}
```

### 4.2 配置管理

**原则**: 使用配置结构体，支持环境变量和配置文件

```go
// ✅ 正确：使用配置结构体
type Config struct {
    Database DatabaseConfig `mapstructure:"database"`
    Server   ServerConfig   `mapstructure:"server"`
}

// 支持环境变量
// DATABASE_HOST=localhost go run main.go
```

---

## 5. 接口层设计

### 5.1 HTTP Handler

**原则**: Handler 应该薄，只负责请求/响应转换

```go
// ✅ 正确：Handler 薄
func (h *UserHandler) CreateUser(w http.ResponseWriter, r *http.Request) {
    var req CreateUserRequest
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
        response.Error(w, http.StatusBadRequest, errors.NewInvalidInputError("invalid request"))
        return
    }

    user, err := h.userService.CreateUser(r.Context(), req)
    if err != nil {
        response.Error(w, http.StatusInternalServerError, err)
        return
    }

    response.Success(w, http.StatusCreated, user)
}
```

### 5.2 中间件使用

**原则**: 使用中间件处理横切关注点

```go
// ✅ 正确：使用中间件
r.Use(middleware.TracingMiddleware(...))
r.Use(middleware.LoggingMiddleware(...))
r.Use(middleware.AuthMiddleware(...))
r.Use(middleware.RateLimitMiddleware(...))
```

---

## 6. 错误处理

### 6.1 错误分类

**原则**: 使用统一的错误处理框架

```go
// ✅ 正确：使用框架错误处理
import "github.com/yourusername/golang/pkg/errors"

// 领域错误
return errors.NewNotFoundError("user not found")

// 应用错误
return errors.NewInvalidInputError("invalid email format")

// 系统错误
return errors.NewInternalError("database connection failed")
```

### 6.2 错误包装

**原则**: 使用 `fmt.Errorf` 和 `%w` 包装错误

```go
// ✅ 正确：包装错误
if err := db.Query(ctx, query); err != nil {
    return fmt.Errorf("failed to query user: %w", err)
}
```

---

## 7. 日志记录

### 7.1 结构化日志

**原则**: 使用结构化日志，包含上下文信息

```go
// ✅ 正确：结构化日志
logger.Info("user created",
    "user_id", user.ID,
    "email", user.Email,
    "duration_ms", duration.Milliseconds(),
)

// ❌ 错误：非结构化日志
log.Printf("user created: %s", user.ID)
```

### 7.2 日志级别

**原则**: 合理使用日志级别

- **Debug**: 详细的调试信息
- **Info**: 重要的业务事件
- **Warn**: 警告信息，不影响功能
- **Error**: 错误信息，需要关注

---

## 8. 测试策略

### 8.1 单元测试

**原则**: 测试领域层和应用层，使用 Mock

```go
// ✅ 正确：使用 Mock 测试应用服务
func TestUserService_CreateUser(t *testing.T) {
    mockRepo := &MockUserRepository{}
    service := NewUserService(mockRepo)

    // 测试逻辑
}
```

### 8.2 集成测试

**原则**: 测试基础设施层和接口层

```go
// ✅ 正确：集成测试使用真实数据库
func TestUserRepository_Integration(t *testing.T) {
    db := setupTestDB(t)
    defer teardownTestDB(t, db)

    repo := NewUserRepository(db)
    // 测试逻辑
}
```

---

## 9. 性能优化

### 9.1 数据库查询

**原则**: 使用索引，避免 N+1 查询

```go
// ✅ 正确：批量查询
users, err := repo.FindByIDs(ctx, ids)

// ❌ 错误：N+1 查询
for _, id := range ids {
    user, _ := repo.FindByID(ctx, id)
}
```

### 9.2 缓存使用

**原则**: 缓存热点数据，设置合理的过期时间

```go
// ✅ 正确：使用缓存
user, err := cache.Get(ctx, key)
if err == nil {
    return user, nil
}

user, err := repo.FindByID(ctx, id)
cache.Set(ctx, key, user, 5*time.Minute)
```

---

## 10. 安全实践

### 10.1 输入验证

**原则**: 始终验证和清理用户输入

```go
// ✅ 正确：验证输入
type CreateUserRequest struct {
    Email string `validate:"required,email"`
    Name  string `validate:"required,min=2,max=50"`
}

if err := validator.Validate(req); err != nil {
    return errors.NewValidationError("invalid input", err)
}
```

### 10.2 认证授权

**原则**: 使用中间件进行认证和授权

```go
// ✅ 正确：使用认证中间件
r.Use(middleware.AuthMiddleware(...))
r.Use(middleware.RequireRole("admin"))
```

---

## 📚 相关文档

- [架构模型与依赖注入完整说明](../architecture/00-架构模型与依赖注入完整说明.md)
- [框架使用示例说明](02-框架使用示例说明.md)
- [框架代码与业务代码说明](03-框架代码与业务代码说明.md)

---

**最后更新**: 2025-01-XX
