...
```mermaid
flowchart TD
    Start([选择 Web 框架]) --> Q1{需要标准库兼容?}
    Q1 -->|是| Q2{需要学习成本低?}
    Q1 -->|否| Q3{需要性能最优?}
    Q2 -->|是| Chi[Chi Router<br/>✅ 选择]
    Q2 -->|否| Gin[Gin<br/>性能优秀]
    Q3 -->|是| Fiber[Fiber<br/>性能最好]
    Q3 -->|否| Echo[Echo<br/>功能丰富]

    Chi --> End([Chi Router<br/>标准库兼容])
    Gin --> End2([Gin])
    Echo --> End3([Echo])
    Fiber --> End4([Fiber])

    style Chi fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
...
```mermaid
flowchart TD
    Start([选择可观测性方案]) --> Q1{需要统一标准?}
    Q1 -->|是| Q2{需要三支柱支持?}
    Q1 -->|否| Q3{需要追踪?}
    Q2 -->|是| OTel[OpenTelemetry<br/>✅ 选择]
    Q2 -->|否| Prometheus[Prometheus<br/>指标监控]
    Q3 -->|是| Q4{需要指标?}
    Q3 -->|否| Jaeger[Jaeger<br/>追踪系统]
    Q4 -->|是| OTel
    Q4 -->|否| Jaeger

    OTel --> End([OpenTelemetry<br/>统一标准])
    Prometheus --> End2([Prometheus])
    Jaeger --> End3([Jaeger])

    style OTel fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
    style End fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
```
...
```mermaid
flowchart TD
    Start([选择消息队列]) --> Q1{使用场景?}
    Q1 -->|高吞吐量| Q2{需要持久化?}
    Q1 -->|IoT 场景| MQTT[MQTT<br/>轻量级协议]
    Q1 -->|低延迟| NATS[NATS<br/>高性能]
    Q2 -->|是| Kafka[Kafka<br/>✅ 选择]
    Q2 -->|否| NATS

    Kafka --> Q3{需要 IoT 支持?}
    Q3 -->|是| Combined[Kafka + MQTT<br/>✅ 组合方案]
    Q3 -->|否| Kafka

    Combined --> End([Kafka + MQTT<br/>组合方案])
    Kafka --> End2([Kafka])
    MQTT --> End3([MQTT])
    NATS --> End4([NATS])

    style Combined fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
    style End fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
```
...
```mermaid
flowchart TD
    Start([选择数据库]) --> Q1{需要 JSON 支持?}
    Q1 -->|是| Q2{需要 ACID 事务?}
    Q1 -->|否| Q3{需要关系型?}
    Q2 -->|是| PostgreSQL[PostgreSQL<br/>✅ 选择]
    Q2 -->|否| MongoDB[MongoDB<br/>文档数据库]
    Q3 -->|是| Q4{需要功能完整?}
    Q3 -->|否| Redis[Redis<br/>缓存数据库]
    Q4 -->|是| PostgreSQL
    Q4 -->|否| MySQL[MySQL<br/>关系型数据库]

    PostgreSQL --> End([PostgreSQL<br/>功能完整])
    MySQL --> End2([MySQL])
    MongoDB --> End3([MongoDB])
    Redis --> End4([Redis])

    style PostgreSQL fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
    style End fill:#4caf50,stroke:#2e7d32,stroke-width:3px,color:#fff
```
