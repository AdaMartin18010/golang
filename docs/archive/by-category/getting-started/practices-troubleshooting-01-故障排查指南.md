...
- [故障排查指南](#故障排查指南)
  ...
  - [3. 性能问题排查](#3-性能问题排查)
- [1. 确认是哪个进程](#1-确认是哪个进程)
- [发现进程PID 12345占用CPU 95%](#发现进程pid-12345占用cpu-95)
- [2. 采集CPU profile](#2-采集cpu-profile)
- [3. 分析热点](#3-分析热点)
- [4. 查看火焰图](#4-查看火焰图)
- [1. 查看监控图表](#1-查看监控图表)
- [发现在15:30开始延迟增加](#发现在1530开始延迟增加)
- [2. 检查日志](#2-检查日志)
- [发现大量"database connection timeout"](#发现大量database-connection-timeout)
- [3. 检查数据库连接](#3-检查数据库连接)
- [4. 查看数据库连接池状态](#4-查看数据库连接池状态)
  - [4. 内存问题排查](#4-内存问题排查)
- [1. 采集多个时间点的heap快照](#1-采集多个时间点的heap快照)
- [2. 对比快照找出增长点](#2-对比快照找出增长点)
- [3. 查看具体分配](#3-查看具体分配)
- [1. 查看goroutine数量](#1-查看goroutine数量)
- [2. 分析堆栈](#2-分析堆栈)
- [3. 找出重复的堆栈](#3-找出重复的堆栈)
  - [5. 并发问题排查](#5-并发问题排查)
- [1. 使用race detector编译运行](#1-使用race-detector编译运行)
- [或在测试中](#或在测试中)
- [2. 查看race报告](#2-查看race报告)
- [1. 获取所有goroutine堆栈](#1-获取所有goroutine堆栈)
- [2. 查找持有锁的goroutine](#2-查找持有锁的goroutine)
  - [6. 网络问题排查](#6-网络问题排查)
- [1. 查看连接状态](#1-查看连接状态)
- [输出: 28000 (过多！)](#输出-28000-过多)
- [2. 查看系统限制](#2-查看系统限制)
- [输出: 32768  60999 (可用端口: 28231)](#输出-32768-60999-可用端口-28231)
- [3. 查看哪些连接](#3-查看哪些连接)
  - [7. 数据库问题排查](#7-数据库问题排查)
- [1. 查看当前连接数](#1-查看当前连接数)
- [显示: 150个连接，大部分Sleep状态](#显示-150个连接大部分sleep状态)
- [2. 查看应用连接池状态](#2-查看应用连接池状态)
- [3. 分析慢查询日志](#3-分析慢查询日志)
  - [8. 实战案例](#8-实战案例)
- [1. 查看监控大盘](#1-查看监控大盘)
- [发现: CPU正常30%, 内存正常, 但QPS下降50%](#发现-cpu正常30-内存正常-但qps下降50)
- [2. 查看错误日志](#2-查看错误日志)
- [发现大量: "Context deadline exceeded"](#发现大量-context-deadline-exceeded)
- [3. 初步判断: 下游服务超时](#3-初步判断-下游服务超时)
- [检查所有下游服务](#检查所有下游服务)
- [输出:](#输出)
- [user: time: 0.05s ✓](#user-time-005s)
- [product: time: 0.08s ✓](#product-time-008s)
- [inventory: time: 8.52s ✗  ← 问题在这里！](#inventory-time-852s-问题在这里)
- [登录inventory服务器](#登录inventory服务器)
- [查看系统资源](#查看系统资源)
- [发现: CPU 100%, 一个进程占用](#发现-cpu-100-一个进程占用)
- [查看进程](#查看进程)
- [PID: 12345](#pid-12345)
- [采集CPU profile](#采集cpu-profile)
- [分析](#分析)
- [输出: checkStock 函数占用95%](#输出-checkstock-函数占用95)
- [重启服务](#重启服务)
- [验证](#验证)
- [time: 0.05s ✓ 恢复正常](#time-005s-恢复正常)
- [监控大盘显示:](#监控大盘显示)
- [- P99延迟: 5s → 50ms ✓](#p99延迟-5s-50ms)
- [- 错误率: 5% → 0.1% ✓](#错误率-5-01)
- [- QPS: 恢复到正常值 ✓](#qps-恢复到正常值)
  - [9. 应急响应](#9-应急响应)
- [快速重启服务](#快速重启服务)
- [回滚到上一版本](#回滚到上一版本)
- [手动扩容](#手动扩容)
