...
- [⚡ 并发优化指南](#并发优化指南)
  - [🎯 概述](#概述)
  - [🔧 并发分析工具](#并发分析工具)
- [运行pprof服务器](#运行pprof服务器)
- [查看Goroutine数量](#查看goroutine数量)
- [生成profile](#生成profile)
- [运行测试with race detector](#运行测试with-race-detector)
- [运行应用with race detector](#运行应用with-race-detector)
- [构建with race detector](#构建with-race-detector)
- [生成CPU profile](#生成cpu-profile)
- [分析](#分析)
- [设置blocking profile](#设置blocking-profile)
- [访问profile](#访问profile)
  - [🚀 优化策略](#优化策略)
  - [🎨 常见模式](#常见模式)
  - [📊 性能基准](#性能基准)
...
func processWithWorkerPool(items []Item) {
    jobs := make(Channel Item, len(items))
    results := make(Channel Result, len(items))
...
func worker(jobs <-Channel Item, results Channel<- Result) {
    for job := range jobs {
        results <- process(job)
    }
}
...
// ✅ 使用缓冲
ch := make(Channel int, 100)  // 缓冲100个元素
...
jobs := make(Channel Job, bufferSize)
```
...
func fanOut(ctx Context.Context, input <-Channel Job, workerCount int) []<-Channel Result {
    channels := make([]<-Channel Result, workerCount)

    for i := 0; i < workerCount; i++ {
        ch := make(Channel Result)
        channels[i] = ch

        go func(out Channel<- Result) {
            defer close(out)
            for job := range input {
                select {
                case <-ctx.Done():
...
func fanIn(ctx Context.Context, channels ...<-Channel Result) <-Channel Result {
    out := make(Channel Result)
    var wg sync.WaitGroup

    for _, ch := range channels {
        wg.Add(1)
        go func(c <-Channel Result) {
            defer wg.Done()
            for result := range c {
                select {
                case <-ctx.Done():
...
```go
func pipeline(ctx Context.Context, input <-Channel int) <-Channel int {
    // Stage 1: 乘以2
...
// 更好: 使用channel
type SafeCounter struct {
    ops Channel func(map[string]int)
}

func NewSafeCounter() *SafeCounter {
    sc := &SafeCounter{
        ops: make(Channel func(map[string]int)),
    }
...
3. **使用buffered Channel**: 减少阻塞
    ...
    ```go
    // 根据吞吐量设置缓冲大小
    jobs := make(Channel Job, workerCount*10)
    ```
