...
- [Go 1.26实时数据处理与流计算完整实战](#go-126实时数据处理与流计算完整实战)
  - [📋 目录](#-目录)
  - [1. 实时数据处理概述](#1-实时数据处理概述)
    - [1.1 批处理 vs 流处理](#11-批处理-vs-流处理)
    - [1.2 流处理核心概念](#12-流处理核心概念)
  - [2. Kafka Streams实现](#2-kafka-streams实现)
    - [2.1 Kafka Streams基础](#21-kafka-streams基础)
    - [2.2 流式转换 (Map/Filter/FlatMap)](#22-流式转换-mapfilterflatmap)
    - [2.3 GroupBy与聚合](#23-groupby与聚合)
  - [3. 流式窗口计算](#3-流式窗口计算)
    - [3.1 Tumbling Window (翻滚窗口)](#31-tumbling-window-翻滚窗口)
    - [3.2 Sliding Window (滑动窗口)](#32-sliding-window-滑动窗口)
    - [3.3 Session Window (会话窗口)](#33-session-window-会话窗口)
  - [4. 状态管理与容错](#4-状态管理与容错)
    - [4.1 Keyed State](#41-keyed-state)
    - [4.2 Checkpoint机制](#42-checkpoint机制)
  - [5. CDC数据捕获](#5-cdc数据捕获)
    - [5.1 Debezium集成](#51-debezium集成)
    - [5.2 Change Data Capture Pipeline](#52-change-data-capture-pipeline)
  - [6. 时序数据处理](#6-时序数据处理)
    - [6.1 InfluxDB集成](#61-influxdb集成)
    - [6.2 时序数据聚合](#62-时序数据聚合)
  - [7. 实时数据管道](#7-实时数据管道)
    - [7.1 完整数据管道](#71-完整数据管道)
    - [7.2 背压处理](#72-背压处理)
  - [8. 性能优化与监控](#8-性能优化与监控)
    - [8.1 性能指标](#81-性能指标)
    - [8.2 性能Benchmark](#82-性能benchmark)
  - [9. 总结](#9-总结)
    - [9.1 最佳实践](#91-最佳实践)
  - [📚 参考资料](#-参考资料)
...
func NewGroupByStream(source <-Channel Event) *GroupByStream {
...
// Process 处理事件并输出聚合结果
func (g *GroupByStream) Process(ctx Context.Context, interval time.Duration) <-Channel map[string]*AggregateState {
    out := make(Channel map[string]*AggregateState)
...
func NewTumblingWindow(size time.Duration) *TumblingWindow {
    return &TumblingWindow{
        ...
        resultCh: make(Channel WindowResult, 100),
    }
}
...
// Start 启动窗口处理
func (w *TumblingWindow) Start(ctx Context.Context) <-Channel WindowResult {
...
// Start 启动窗口处理
func (w *SlidingWindow) Start(ctx Context.Context) <-Channel WindowResult {
...
type SessionWindow struct {
    ...
    mu       sync.RWMutex
    resultCh Channel SessionResult
}
...
func NewSessionWindow(gap time.Duration) *SessionWindow {
    return &SessionWindow{
        ...
        resultCh: make(Channel SessionResult, 100),
    }
}
...
// Start 启动会话检查
func (w *SessionWindow) Start(ctx Context.Context) <-Channel SessionResult {
...
type CDCSource interface {
    Read(ctx Context.Context) (<-Channel DebeziumEvent, error)
}
...
type TimeSeriesAggregator struct {
    ...
    mu       sync.RWMutex
    resultCh Channel AggregateResult
}
...
func NewTimeSeriesAggregator(interval time.Duration) *TimeSeriesAggregator {
    return &TimeSeriesAggregator{
        ...
        resultCh: make(Channel AggregateResult, 100),
    }
}
...
// Start 启动聚合
func (a *TimeSeriesAggregator) Start(ctx Context.Context) <-Channel AggregateResult {
...
type Source interface {
    Read(ctx Context.Context) (<-Channel interface{}, error)
}

// Operator 算子
type Operator interface {
    Process(ctx Context.Context, input <-Channel interface{}) (<-Channel interface{}, error)
}

// Sink 数据输出
type Sink interface {
    Write(ctx Context.Context, data <-Channel interface{}) error
}
...
func (o *FilterOperator) Process(ctx Context.Context, input <-Channel interface{}) (<-Channel interface{}, error) {
...
func (o *MapOperator) Process(ctx Context.Context, input <-Channel interface{}) (<-Channel interface{}, error) {
    output := make(Channel interface{})
...
func (o *BackpressureOperator) Process(ctx Context.Context, input <-Channel interface{}) (<-Channel interface{}, error) {
    output := make(Channel interface{}, o.bufferSize)
...
func (o *RateLimitOperator) Process(ctx Context.Context, input <-Channel interface{}) (<-Channel interface{}, error) {
    output := make(Channel interface{})
...
func BenchmarkStreamProcessing(b *testing.B) {
    ctx := context.Background()
    source := make(Channel Event, 1000)
...
**恭喜!** 🎉 您已掌握Go 1.26实时数据处理与流计算的完整实战技能!
