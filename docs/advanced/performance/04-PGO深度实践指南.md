...
- [Go 1.23+ PGO深度实践指南](#go-123-pgo深度实践指南)
  ...
  - [2. PGO快速开始](#2-pgo快速开始)
- [1. 正常编译](#1-正常编译)
- [2. 运行并收集Profile](#2-运行并收集profile)
- [或使用benchmark](#或使用benchmark)
- [3. 使用Profile重新编译](#3-使用profile重新编译)
- [或使用默认文件名](#或使用默认文件名)
- [1. 标准编译](#1-标准编译)
- [2. 运行收集Profile](#2-运行收集profile)
- [3. PGO编译](#3-pgo编译)
- [4. 性能对比](#4-性能对比)
- [收集30秒的CPU Profile](#收集30秒的cpu-profile)
- [运行benchmark并收集Profile](#运行benchmark并收集profile)
- [编译基准版本](#编译基准版本)
- [运行并收集Profile](#运行并收集profile)
- [PGO编译](#pgo编译)
- [性能对比](#性能对比)
- [检查二进制大小](#检查二进制大小)
- [安装benchstat](#安装benchstat)
- [运行baseline benchmark](#运行baseline-benchmark)
- [使用PGO重新编译](#使用pgo重新编译)
- [运行PGO benchmark](#运行pgo-benchmark)
- [对比结果](#对比结果)
  - [3. Profile收集策略](#3-profile收集策略)
  - [4. PGO优化原理](#4-pgo优化原理)
  - [5. Go 1.23 PGO增强](#5-go-123-pgo增强)
- [Go 1.23 新增选项](#go-123-新增选项)
- [1. PGO详细输出](#1-pgo详细输出)
- [2. PGO统计信息](#2-pgo统计信息)
- [3. 禁用特定PGO优化](#3-禁用特定pgo优化)
  - [6. 实战案例](#6-实战案例)
- [1. 编译基准版本](#1-编译基准版本)
- [2. 启动服务器收集Profile](#2-启动服务器收集profile)
- [等待服务器启动](#等待服务器启动)
- [3. 生成负载](#3-生成负载)
- [等待服务器退出（30秒）](#等待服务器退出30秒)
- [4. 使用PGO编译](#4-使用pgo编译)
- [5. 性能对比](#5-性能对比)
- [6. 对比结果](#6-对比结果)
  - [7. 最佳实践](#7-最佳实践)
- [.github/workflows/pgo.yml](#githubworkflowspgoyml)
- [从生产环境下载最新Profile](#从生产环境下载最新profile)
  - [9. 参考资源](#9-参考资源)
