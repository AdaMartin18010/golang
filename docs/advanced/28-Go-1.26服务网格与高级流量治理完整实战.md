...
- [Go 1.26服务网格与高级流量治理完整实战](#go-1253服务网格与高级流量治理完整实战)
  - [1. Service Mesh概述](#1-service-mesh概述)
  - [2. Istio集成实战](#2-istio集成实战)
- [安装Istio CLI](#安装istio-cli)
- [安装Istio到Kubernetes](#安装istio到kubernetes)
- [启用自动注入](#启用自动注入)
- [验证安装](#验证安装)
- [deployments/user-service.yaml](#deploymentsuser-serviceyaml)
- [istio/virtualservice.yaml](#istiovirtualserviceyaml)
- [路由规则1: Header匹配](#路由规则1-header匹配)
- [路由规则2: 金丝雀发布 (90% v1, 10% v2)](#路由规则2-金丝雀发布-90-v1-10-v2)
- [超时配置](#超时配置)
- [重试策略](#重试策略)
- [istio/destinationrule.yaml](#istiodestinationruleyaml)
- [连接池配置](#连接池配置)
- [负载均衡](#负载均衡)
- [异常检测 (熔断)](#异常检测-熔断)
- [子集定义](#子集定义)
  - [3. Linkerd轻量级方案](#3-linkerd轻量级方案)
- [安装Linkerd CLI](#安装linkerd-cli)
- [验证集群](#验证集群)
- [安装Linkerd控制平面](#安装linkerd控制平面)
- [验证安装](#验证安装)
- [查看Dashboard](#查看dashboard)
- [deployments/product-service.yaml](#deploymentsproduct-serviceyaml)
- [启用Linkerd自动注入](#启用linkerd自动注入)
- [Linkerd配置](#linkerd配置)
- [linkerd/trafficsplit.yaml](#linkerdtrafficsplityaml)
  - [4. 高级流量治理](#4-高级流量治理)
- [istio/blue-green.yaml](#istioblue-greenyaml)
- [istio/ab-testing.yaml](#istioab-testingyaml)
- [规则1: 测试组用户 -> Algorithm A](#规则1-测试组用户-algorithm-a)
- [规则2: 对照组用户 -> Algorithm B](#规则2-对照组用户-algorithm-b)
- [规则3: 其他用户 -> 随机分配 (50/50)](#规则3-其他用户-随机分配-5050)
- [istio/fault-injection.yaml](#istiofault-injectionyaml)
- [延迟注入](#延迟注入)
- [错误注入](#错误注入)
  - [5. 安全通信与mTLS](#5-安全通信与mtls)
- [istio/peer-authentication.yaml](#istiopeer-authenticationyaml)
- [针对特定服务配置](#针对特定服务配置)
- [istio/authorization-policy.yaml](#istioauthorization-policyyaml)
- [规则1: 允许frontend调用](#规则1-允许frontend调用)
- [规则2: 允许admin服务所有操作](#规则2-允许admin服务所有操作)
- [规则3: 拒绝外部直接访问](#规则3-拒绝外部直接访问)
- [istio/request-authentication.yaml](#istiorequest-authenticationyaml)
- [配合授权策略](#配合授权策略)
  - [6. 多集群服务网格](#6-多集群服务网格)
- [1. 生成CA证书 (共享根证书)](#1-生成ca证书-共享根证书)
- [生成根CA](#生成根ca)
- [为cluster1生成中间CA](#为cluster1生成中间ca)
- [为cluster2生成中间CA](#为cluster2生成中间ca)
- [2. 在cluster1安装Istio](#2-在cluster1安装istio)
- [3. 安装East-West Gateway](#3-安装east-west-gateway)
- [4. 暴露服务](#4-暴露服务)
- [5. 在cluster2重复相同步骤...](#5-在cluster2重复相同步骤)
- [cluster1/serviceentry.yaml](#cluster1serviceentryyaml)
- [multi-cluster-routing.yaml](#multi-cluster-routingyaml)
- [优先路由到本地集群](#优先路由到本地集群)
- [20%流量到cluster2](#20流量到cluster2)
  - [7. 可观测性集成](#7-可观测性集成)
- [安装Kiali](#安装kiali)
- [访问Kiali Dashboard](#访问kiali-dashboard)
- [Kiali提供:](#kiali提供)
- [1. 服务拓扑图](#1-服务拓扑图)
- [2. 流量流向](#2-流量流向)
- [3. 健康状态](#3-健康状态)
...
- [全局Sidecar资源配置](#全局sidecar资源配置)
- [并发配置](#并发配置)
- [针对高流量服务的Sidecar优化](#针对高流量服务的sidecar优化)
- [...](#)
- [限制Sidecar只监听需要的服务](#限制sidecar只监听需要的服务)
- [不监听所有服务,减少配置同步开销](#不监听所有服务减少配置同步开销)
  - [9. 总结](#9-总结)
- [Service Mesh健康指标](#service-mesh健康指标)
- [数据平面](#数据平面)
- [控制平面](#控制平面)
  - [📚 参考资料](#参考资料)
...
**恭喜!** 🎉 您已掌握Go 1.26 Service Mesh与高级流量治理的完整实战技能!
