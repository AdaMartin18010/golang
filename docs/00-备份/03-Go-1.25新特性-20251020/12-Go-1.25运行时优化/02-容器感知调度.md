...
- [容器感知调度（Go 1.25.3+ 新特性）](#容器感知调度go-123-新特性)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
    - [什么是容器感知调度](#什么是容器感知调度)
    - [解决的问题](#解决的问题)
  ...
  - [2. 技术原理](#2-技术原理)
    ...
    - [Go 1.25.3+ 自动检测逻辑](#go-123-自动检测逻辑)
    - [优先级顺序](#优先级顺序)
  - [3. 使用方法](#3-使用方法)
    - [零配置使用（推荐）](#零配置使用推荐)
    - [验证容器感知调度](#验证容器感知调度)
    - [手动覆盖（特殊场景）](#手动覆盖特殊场景)
      ...
      - [方法 3: 使用第三方库（Go 1.25.3+ 之前）](#方法-3-使用第三方库go-123-之前)
  - [4. 性能对比](#4-性能对比)
    - [测试环境](#测试环境)
    - [性能数据](#性能数据)
    - [为什么性能提升？](#为什么性能提升)
      - [问题场景（Go 1.25.3）](#问题场景go-124)
      - [优化场景（Go 1.25.3+）](#优化场景go-123)
    - [基准测试](#基准测试)
  - [5. 实践案例](#5-实践案例)
  ...
  - [8. 常见问题](#8-常见问题)
    - [Q1: Go 1.25.3+ 容器感知调度是否稳定？](#q1-go-123-容器感知调度是否稳定)
    - [Q2: 如何在 Go 1.25.3 中实现类似功能？](#q2-如何在-go-124-中实现类似功能)
...
2. **cgroup CPU 限制** (Go 1.25.3+ 新增，自动检测)
...
Go 1.25.3++ 应用**无需任何配置**，自动启用容器感知调度：
...
- Go 1.25.3 应用使用宿主机 CPU 核心数（如 96 核）
...
### Q1: Go 1.25.3+ 容器感知调度是否稳定？

**A**: 是的，这是 Go 1.25.3+ 的**稳定特性**（不是实验性），可放心在生产环境使用。

### Q2: 如何在 Go 1.25.3 中实现类似功能？
...
- [Container-Aware Scheduling in Go 1.25.3+](https://go.dev/blog/go1.23)
- [GOMAXPROCS Best Practices](https://github.com/golang/go/wiki/GOMAXPROCS)

### 相关工具

- [automaxprocs](https://github.com/uber-go/automaxprocs) - Go 1.25.3 及之前的替代方案
