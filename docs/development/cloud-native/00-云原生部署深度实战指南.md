...
- [Pod选择器](#pod选择器)
- [端口映射](#端口映射)
- [Session亲和性](#session亲和性)
- [service-nodeport.yaml（用于外部访问）](#service-nodeportyaml用于外部访问)
- [service-loadbalancer.yaml（云平台）](#service-loadbalanceryaml云平台)
  - [第三部分：ConfigMap与Secret](#第三部分configmap与secret)
- [configmap.yaml](#configmapyaml)
- [键值对](#键值对)
- [文件内容](#文件内容)
- [secret.yaml](#secretyaml)
- [或使用base64编码](#或使用base64编码)
- [在Pod中使用ConfigMap和Secret](#在pod中使用configmap和secret)
- [方式1：环境变量](#方式1环境变量)
- [方式2：挂载为文件](#方式2挂载为文件)
  - [第四部分：Ingress路由](#第四部分ingress路由)
- [ingress.yaml](#ingressyaml)
- [Nginx Ingress注解](#nginx-ingress注解)
- [CORS配置](#cors配置)
- [限流](#限流)
- [超时](#超时)
