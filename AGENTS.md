...
- CI 门：`.github/workflows/quality-gates.yml`（死链检查 `scripts/check-unfixed-links.ps1`（Exit 1 强制）+ 文档质量检查 `scripts/check_quality.ps1`（`-FailOnHigh`：高严重性问题 Exit 1 强制门，存量高问题已清零）+ gofmt（强制，范围 `cmd/` `internal/` `pkg/`））；其余 workflow 见 go-test / go-lint / go-fix / docs-check / knowledge-tracker 等。
