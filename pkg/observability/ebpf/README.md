# eBPF 可观测性实现

**版本**: v2.0  
**更新日期**: 2025-12-03  
**使用库**: github.com/cilium/ebpf (最成熟的Go eBPF库)

---

## 📋 功能概述

本模块使用 **Cilium eBPF** 库实现系统级可观测性，提供：

1. **系统调用追踪** - 追踪应用的系统调用
2. **网络监控** - 监控网络包和连接
3. **性能分析** - CPU、内存、I/O性能
4. **安全监控** - 检测异常行为

---

## 🏗️ 架构设计

### 分层结构

```text
┌─────────────────────────────────────┐
│   OpenTelemetry Metrics/Traces     │  ← 输出层
├─────────────────────────────────────┤
│   eBPF Collector (Go)              │  ← 数据处理层
├─────────────────────────────────────┤
│   eBPF Maps (Kernel)               │  ← 数据缓冲层
├─────────────────────────────────────┤
│   eBPF Programs (C)                │  ← 数据采集层
├─────────────────────────────────────┤
│   Kernel Events                     │  ← 事件源
└─────────────────────────────────────┘
```

### 数据流

```text
Kernel Event → eBPF Program → eBPF Map → Go Collector → OTLP → Backend
```

---

## 🚀 实现计划

### Phase 1: 系统调用追踪（当前）

**文件结构**:
```
pkg/observability/ebpf/
├── collector.go           # 主收集器（已有框架）
├── syscall_tracer.go      # 系统调用追踪器（待实现）
├── programs/
│   ├── syscall.bpf.c     # eBPF C程序（待实现）
│   └── syscall.go        # Go绑定（bpf2go生成）
└── README.md             # 本文档
```

**实现步骤**:
1. ✅ 添加 Cilium eBPF 依赖
2. ⏳ 编写 eBPF C 程序追踪系统调用
3. ⏳ 使用 bpf2go 生成 Go 绑定
4. ⏳ 实现 Go 收集器读取 eBPF map
5. ⏳ 集成到 OpenTelemetry

### Phase 2: 网络监控（下一步）

**功能**:
- TCP 连接监控
- 网络包统计
- 延迟测量

### Phase 3: 性能分析（未来）

**功能**:
- CPU 火焰图
- 内存分配追踪
- I/O 性能分析

---

## 📚 技术选型

### Cilium eBPF vs 其他方案

| 方案 | 优势 | 劣势 | 评分 |
|------|------|------|------|
| **Cilium eBPF** | 纯Go、类型安全、活跃维护 | 需要编写C程序 | ⭐⭐⭐⭐⭐ |
| libbpfgo | 功能完整 | CGO依赖、复杂 | ⭐⭐⭐⭐ |
| gobpf | 简单 | 不维护、过时 | ⭐⭐ |

**选择 Cilium eBPF 的原因**:
1. ✅ Kubernetes 网络核心项目，成熟稳定
2. ✅ 纯 Go 实现，无 CGO 依赖
3. ✅ 类型安全，编译时检查
4. ✅ 活跃维护，社区支持好
5. ✅ 完整的文档和示例

---

## 🔧 开发环境要求

### Linux 内核要求

- **最低版本**: Linux 4.18+
- **推荐版本**: Linux 5.10+ (LTS)
- **最佳版本**: Linux 6.1+ (最新特性)

### 开发工具

```bash
# 安装 LLVM/Clang (编译 eBPF 程序)
sudo apt-get install clang llvm

# 安装 bpf2go (生成 Go 绑定)
go install github.com/cilium/ebpf/cmd/bpf2go@latest

# 安装内核头文件
sudo apt-get install linux-headers-$(uname -r)
```

---

## 📖 使用示例

### 基础使用

```go
package main

import (
    "context"
    "log"
    
    "github.com/yourusername/golang/pkg/observability/ebpf"
    "go.opentelemetry.io/otel"
)

func main() {
    // 创建 OTLP tracer 和 meter
    tracer := otel.Tracer("ebpf-collector")
    meter := otel.Meter("ebpf-collector")
    
    // 创建 eBPF 收集器
    collector, err := ebpf.NewCollector(ebpf.Config{
        Tracer:                 tracer,
        Meter:                  meter,
        Enabled:                true,
        EnableSyscallTracking:  true,
        EnableNetworkMonitoring: true,
    })
    if err != nil {
        log.Fatal(err)
    }
    
    // 启动收集
    if err := collector.Start(); err != nil {
        log.Fatal(err)
    }
    defer collector.Stop()
    
    // 应用运行...
    select {}
}
```

---

## 🎯 下一步工作

### 立即任务

1. **实现系统调用追踪器**
   - 编写 `syscall.bpf.c`
   - 实现 `syscall_tracer.go`
   - 集成到 collector

2. **添加测试**
   - 单元测试
   - 集成测试（需要 Linux 环境）

3. **完善文档**
   - 使用指南
   - 故障排查
   - 性能调优

---

## 📚 参考资源

- [Cilium eBPF 官方文档](https://ebpf-go.dev/)
- [eBPF 入门教程](https://ebpf.io/what-is-ebpf/)
- [BPF CO-RE](https://nakryiko.com/posts/bpf-portability-and-co-re/)

---

**状态**: 🔄 重构中  
**目标**: 使用 Cilium eBPF 实现真正的系统级监控
