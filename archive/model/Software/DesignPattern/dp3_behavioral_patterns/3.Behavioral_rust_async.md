...
<!-- TOC START -->
- [ ](#1-1-1-1-1-1-1-行为型模式behavioral-patterns)
  - [1. ](#异步实现)
    - [1.1 ](#1-责任链模式（chain-of-responsibility-pattern）)
    - [1.2 ](#2-命令模式（command-pattern）)
    - [1.3 ](#3-解释器模式（interpreter-pattern）)
    - [1.4 ](#4-迭代器模式（iterator-pattern）)
    - [1.5 ](#5-中介者模式（mediator-pattern）)
