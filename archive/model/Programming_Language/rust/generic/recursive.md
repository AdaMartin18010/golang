...
- [Rust 的递归类型表示方法](#rust-的递归类型表示方法)
  - [1.1 目录](#11-目录)
  - [1.2 1. 使用 `Box<T>` 实现递归类型](#12-1-使用-boxt-实现递归类型)
  - [1.3 2. 使用 `Rc<T>` 和 `RefCell<T>` 实现递归类型](#13-2-使用-rct-和-refcellt-实现递归类型)
  - [1.4 3. 使用 `Arc<T>` 实现线程安全的递归类型](#14-3-使用-arct-实现线程安全的递归类型)
  - [1.5 递归类型的原理](#15-递归类型的原理)
  - [1.6 如何有效利用递归类型](#16-如何有效利用递归类型)
    - [1.6.1 1. **使用 `Box<T>` 避免栈溢出**](#161-1-使用-boxt-避免栈溢出)
    - [1.6.2 2. **使用 `Rc<T>` 和 `RefCell<T>` 实现共享和内部可变性**](#162-2-使用-rct-和-refcellt-实现共享和内部可变性)
    - [1.6.3 3. **使用 `Arc<T>` 实现线程安全的递归类型**](#163-3-使用-arct-实现线程安全的递归类型)
  - [1.7 如何提高递归类型的效率](#17-如何提高递归类型的效率)
    - [1.7.1 1. **使用尾递归优化**](#171-1-使用尾递归优化)
    - [1.7.2 2. **使用迭代代替递归**](#172-2-使用迭代代替递归)
    - [1.7.3 3. **使用缓存和记忆化搜索**](#173-3-使用缓存和记忆化搜索)
  - [1.8 总结](#18-总结)
