...
// 注意：Experience类型已在types.go中定义，这里不再重复定义
...
func (le *LearningEngine) updateKnowledgeBase(experience Experience) {
	// 简化处理：使用Input的Data作为State
	if inputData, ok := experience.Input.Data.(map[string]interface{}); ok {
		// 更新事实
	...
	if experience.Reward > 0 {
		rule := Rule{
			ID:         generateRuleID(),
			Condition:  map[string]interface{}{"input_type": experience.Input.Type},
			Action:     experience.Output.Type,
...
func (le *LearningEngine) detectPatterns(experience Experience) {
	...
	if pattern, exists := le.knowledgeBase.patterns[patternKey]; exists {
		...
		le.knowledgeBase.patterns[patternKey] = Pattern{
			ID:          patternKey,
			Sequence:    []string{experience.Input.Type, experience.Output.Type},
			Frequency:   1,
...
func (le *LearningEngine) trainQLearningModel(batch []Experience) error {
	...
	// 简化的Q-Learning更新（使用Input/Output作为状态）
	for _, exp := range batch {
		stateKey := exp.Input.ID
		nextStateKey := exp.Output.ID
...
func (le *LearningEngine) trainSupervisedModel(batch []Experience) error {
	...
	for _, exp := range batch {
		...
		features := map[string]float64{
			"input_type_len": float64(len(exp.Input.Type)),
			"reward":         exp.Reward,
		}
