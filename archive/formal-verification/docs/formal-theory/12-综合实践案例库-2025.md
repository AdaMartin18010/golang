...
- [Go 1.25.3 综合实践案例库](#go-1253-综合实践案例库)
  - [📋 目录](#-目录)
...
func (c *Crawler) Crawl(ctx Context.Context, startURL string) <-Channel Result {
    // 任务队列
...
func (c *Crawler) worker(ctx Context.Context, tasks <-Channel Task) {
    for {
        select {
        case <-ctx.Done():
            return
...
func (c *Crawler) processTask(ctx Context.Context, task Task, tasks Channel<- Task) {
    // 检查是否已访问
...
func NewWorkerPool(workers int) *WorkerPool {
    return &WorkerPool{
        workers: workers,
        jobs:    make(Channel Job, workers*2),
        results: make(Channel Result, workers*2),
    }
}
...
func NewPriorityWorkerPool(workers int) *PriorityWorkerPool {
    return &PriorityWorkerPool{
        workers:  workers,
        jobs:     make(Channel PriorityJob, workers*2),
        results:  make(Channel Result, workers*2),
    }
}
...
type PubSub struct {
    subscribers map[string][]Channel Message
    mu          sync.RWMutex
    bufferSize  int
}
...
func NewPubSub(bufferSize int) *PubSub {
    return &PubSub{
        subscribers: make(map[string][]Channel Message),
        bufferSize:  bufferSize,
    }
}

func (ps *PubSub) Subscribe(topic string) <-Channel Message {
    ps.mu.Lock()
    defer ps.mu.Unlock()

    ch := make(Channel Message, ps.bufferSize)
    ps.subscribers[topic] = append(ps.subscribers[topic], ch)

    return ch
}

func (ps *PubSub) Unsubscribe(topic string, ch <-Channel Message) {
    ps.mu.Lock()
    defer ps.mu.Unlock()
...
func (fps *FilteredPubSub) SubscribeWithFilter(topic string, filter Filter) <-Channel Message {
    fps.mu.Lock()
