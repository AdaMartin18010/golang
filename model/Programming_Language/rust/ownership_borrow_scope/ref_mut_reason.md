# 1 1 1 1 1 1 1 Rust 中的引用、借用与所有权：概念关系与多层级引用分析

<!-- TOC START -->
- [1 1 1 1 1 1 1 Rust 中的引用、借用与所有权：概念关系与多层级引用分析](#1-1-1-1-1-1-1-rust-中的引用、借用与所有权：概念关系与多层级引用分析)
  - [1.1 一、基本概念定义](#一、基本概念定义)
    - [1.1.1 关键字与操作符](#关键字与操作符)
    - [1.1.2 特征与方法](#特征与方法)
  - [1.2 二、概念之间的关系](#二、概念之间的关系)
    - [1.2.1 引用与借用的关系](#引用与借用的关系)
    - [1.2.2 可变性与引用的关系](#可变性与引用的关系)
    - [1.2.3 `ref` 与 `&` 的关系](#ref-与-&-的关系)
    - [1.2.4 `Borrow` 与 `AsRef` 的关系](#borrow-与-asref-的关系)
    - [1.2.5 `to_owned` 与 `clone` 的关系](#to_owned-与-clone-的关系)
  - [1.3 三、多层级引用与解引用](#三、多层级引用与解引用)
    - [1.3.1 引用的层级关系](#引用的层级关系)
    - [1.3.2 自动解引用与方法调用](#自动解引用与方法调用)
    - [1.3.3 多层级可变引用](#多层级可变引用)
  - [1.4 四、等价关系与推理](#四、等价关系与推理)
    - [1.4.1 模式匹配中的 `ref` 与 `&` 等价关系](#模式匹配中的-ref-与-&-等价关系)
    - [1.4.2 多层解引用的等价关系](#多层解引用的等价关系)
    - [1.4.3 借用方法的等价关系](#借用方法的等价关系)
  - [1.5 五、借用检查规则与证明](#五、借用检查规则与证明)
    - [1.5.1 基本借用规则](#基本借用规则)
    - [1.5.2 借用检查与生命周期](#借用检查与生命周期)
    - [1.5.3 非词法生命周期（NLL）](#非词法生命周期（nll）)
  - [1.6 六、复杂示例与分析](#六、复杂示例与分析)
    - [1.6.1 多层级引用与可变性](#多层级引用与可变性)
    - [1.6.2 自引用结构体与生命周期](#自引用结构体与生命周期)
    - [1.6.3 借用与所有权转移](#借用与所有权转移)
    - [1.6.4 `to_owned` 与 `Borrow` 的相互作用](#to_owned-与-borrow-的相互作用)
  - [1.7 七、等价关系的形式化证明](#七、等价关系的形式化证明)
    - [1.7.1 `ref` 与 `&` 的等价性证明](#ref-与-&-的等价性证明)
    - [1.7.2 多层解引用的等价性证明](#多层解引用的等价性证明)
    - [1.7.3 借用方法的等价性证明](#借用方法的等价性证明)
  - [1.8 八、总结与最佳实践](#八、总结与最佳实践)
    - [1.8.1 引用与借用的关键概念](#引用与借用的关键概念)
    - [1.8.2 多层级引用的最佳实践](#多层级引用的最佳实践)
    - [1.8.3 借用与所有权转换的最佳实践](#借用与所有权转换的最佳实践)
    - [1.8.4 借用检查器的工作原理](#借用检查器的工作原理)
<!-- TOC END -->
