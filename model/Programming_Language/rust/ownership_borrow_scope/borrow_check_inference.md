# 1 1 1 1 1 1 1 Borrow Checker

<!-- TOC START -->
- [1 1 1 1 1 1 1 Borrow Checker](#1-1-1-1-1-1-1-borrow-checker)
  - [1.1 path](#path)
  - [1.2 权限的定义](#权限的定义)
    - [1.2.1 可读（Readable, R）](#可读（readable-r）)
    - [1.2.2 可写（Writable, W）](#可写（writable-w）)
    - [1.2.3 可拥有（Ownable, O）](#可拥有（ownable-o）)
  - [1.3 权限的变化](#权限的变化)
    - [1.3.1 变量初始化](#变量初始化)
    - [1.3.2 不可变引用](#不可变引用)
    - [1.3.3 可变引用](#可变引用)
    - [1.3.4 引用结束](#引用结束)
    - [1.3.5 变量移动（Move）](#变量移动（move）)
    - [1.3.6 权限的可视化](#权限的可视化)
  - [1.4 引用的作用域定义](#引用的作用域定义)
    - [1.4.1 引用的生命周期（Lifetime）](#引用的生命周期（lifetime）)
    - [1.4.2 引用的作用域范围](#引用的作用域范围)
    - [1.4.3 引用的作用域与变量的作用域](#引用的作用域与变量的作用域)
    - [1.4.4 引用的作用域与函数调用](#引用的作用域与函数调用)
    - [1.4.5 引用的作用域与返回值](#引用的作用域与返回值)
    - [1.4.6 绕过借用检查器的合法方式](#绕过借用检查器的合法方式)
      - [1.4.6.1 使用 unsafe 块](#使用-unsafe-块)
      - [1.4.6.2 使用 std::cell 和 std::rc 模块](#使用-stdcell-和-stdrc-模块)
      - [1.4.6.3 使用生命周期注解](#使用生命周期注解)
    - [1.4.7 绕过借用检查器的不推荐方式](#绕过借用检查器的不推荐方式)
      - [1.4.7.1 使用 transmute](#使用-transmute)
      - [1.4.7.2 使用 std::ptr 模块](#使用-stdptr-模块)
<!-- TOC END -->
