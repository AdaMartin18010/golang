...
- [在 Rust 中，泛型参数的使用存在以下限制](#在-rust-中泛型参数的使用存在以下限制)
  - [1.1 目录](#11-目录)
  - [1.2 1. **类型限制**](#12-1-类型限制)
    - [1.2.1 2. **生命周期限制**](#121-2-生命周期限制)
    - [1.2.2 3. **`Sized` 限制**](#122-3-sized-限制)
    - [1.2.3 4. **实例化限制**](#123-4-实例化限制)
    - [1.2.4 5. **依赖类型和资源管理**](#124-5-依赖类型和资源管理)
    - [1.2.5 6. **编译器实现限制**](#125-6-编译器实现限制)
