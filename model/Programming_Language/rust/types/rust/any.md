...
- [1 1 1 1 1 1 1 Any 类型与显式类型转换](#1-1-1-1-1-1-1-any-类型与显式类型转换)
  - [1.1 1. `Any` 类型](#11-1-any-类型)
    - [1.1.1 定义与用途](#111-定义与用途)
    - [1.1.2 示例](#112-示例)
  - [1.2 2. 显式类型转换](#12-2-显式类型转换)
    - [1.2.1 主要转换方式](#121-主要转换方式)
    - [1.2.2 (1) 使用 `as` 操作符](#122-1-使用-as-操作符)
    - [1.2.3 (2) 利用 `From`/`Into` 与 `TryFrom`/`TryInto` Trait](#123-2-利用-frominto-与-tryfromtryinto-trait)
    - [1.2.4 (3) 自动解引用转换（Deref Coercion）](#124-3-自动解引用转换deref-coercion)
  - [1.3 总结](#13-总结)
    - [1.3.1 解释](#131-解释)
<!-- TOC END -->
