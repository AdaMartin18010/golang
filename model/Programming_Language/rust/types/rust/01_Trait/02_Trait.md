...
<!-- TOC START -->
- [1.1 Trait](#11-trait)
  - [1.1.1 1. **Copy** - 允许类型通过简单的位拷贝进行复制。如果一个类型实现了 Copy trait，那么在赋值或函数参数传递时，它的值将被复制而不是移动](#111-1-copy---允许类型通过简单的位拷贝进行复制如果一个类型实现了-copy-trait那么在赋值或函数参数传递时它的值将被复制而不是移动)
  - [1.1.2 2. **Clone** - 提供一个显式的复制操作。Clone trait 要求类型也实现了 Copy trait](#112-2-clone---提供一个显式的复制操作clone-trait-要求类型也实现了-copy-trait)
  - [1.1.3 3. **Debug** - 允许使用 `{:?}` 格式化占位符打印类型](#113-3-debug---允许使用--格式化占位符打印类型)
  - [1.1.4 4. **PartialEq** - 允许使用 `==` 运算符比较类型](#114-4-partialeq---允许使用--运算符比较类型)
  - [1.1.5 5. **Eq** - 一个标记 trait，表明类型可以被比较，并且相等性比较是可交换的](#115-5-eq---一个标记-trait表明类型可以被比较并且相等性比较是可交换的)
  - [1.1.6 6. **PartialOrd** - 允许使用 `<`, `>`, `<=`, `>=` 运算符进行部分排序比较](#116-6-partialord---允许使用-----运算符进行部分排序比较)
  - [1.1.7 7. **Ord** - 一个标记 trait，表明类型可以进行排序比较，并且是 PartialOrd 的超集](#117-7-ord---一个标记-trait表明类型可以进行排序比较并且是-partialord-的超集)
  - [1.1.8 8. **Hash** - 允许类型被哈希，通常用于存储在哈希表中](#118-8-hash---允许类型被哈希通常用于存储在哈希表中)
  - [1.1.9 9. **Default** - 提供一个创建类型默认值的方法](#119-9-default---提供一个创建类型默认值的方法)
  - [1.1.10 10. **Iterator** - 定义了迭代器的行为，允许使用 `.next()` 方法和其他迭代器适配器](#1110-10-iterator---定义了迭代器的行为允许使用-next-方法和其他迭代器适配器)
  - [1.1.11 11. **Extend** 和 **FromIterator** - 允许集合从迭代器中扩展或填充](#1111-11-extend-和-fromiterator---允许集合从迭代器中扩展或填充)
  - [1.1.12 12. **AsRef** 和 **AsMut** - 允许隐式地将类型转换为对引用的转换](#1112-12-asref-和-asmut---允许隐式地将类型转换为对引用的转换)
  - [1.1.13 13. **Into** 和 **From** - 允许隐式地将一个类型转换为另一个类型](#1113-13-into-和-from---允许隐式地将一个类型转换为另一个类型)
  - [1.1.14 14. **Drop** - 定义了当值离开作用域时执行的清理逻辑](#1114-14-drop---定义了当值离开作用域时执行的清理逻辑)
  - [1.1.15 15. **Fn**, **FnMut**, 和 **FnOnce** - 这些 trait 定义了闭包的三种捕获方式](#1115-15-fn-fnmut-和-fnonce---这些-trait-定义了闭包的三种捕获方式)
  - [1.1.16 16. **Send** 和 **Sync** - 这些标记 trait 表明类型可以安全地在多线程之间传送或共享](#1116-16-send-和-sync---这些标记-trait-表明类型可以安全地在多线程之间传送或共享)
  - [1.1.17 17. **Error** - 为错误类型提供了一个统一的接口](#1117-17-error---为错误类型提供了一个统一的接口)
  - [1.1.18 18. **Display** 和 **Format** - 允许使用 `{}` 格式化占位符进行格式化](#1118-18-display-和-format---允许使用--格式化占位符进行格式化)
  - [1.1.19 19. **SliceConcatExt** - 允许对切片进行连接操作](#1119-19-sliceconcatext---允许对切片进行连接操作)
  - [1.1.20 20. **Add**, **Sub**, **Mul**, **Div** 等算术运算 trait](#1120-20-add-sub-mul-div-等算术运算-trait)
  - [1.1.21 21. **Iterator** 相关的 trait，如 **Iterator**, **ExactSizeIterator**, **DoubleEndedIterator** 等](#1121-21-iterator-相关的-trait如-iterator-exactsizeiterator-doubleendediterator-等)
  - [1.1.22 分类](#1122-分类)
    - [1.1.22.1 1. **自动 trait**](#11221-1-自动-trait)
    - [1.1.22.2 2. **基本 trait**](#11222-2-基本-trait)
    - [1.1.22.3 3. **比较 trait**](#11223-3-比较-trait)
    - [1.1.22.4 4. **算术 trait**](#11224-4-算术-trait)
    - [1.1.22.5 5. **布尔运算 trait**](#11225-5-布尔运算-trait)
    - [1.1.22.6 6. **内存管理 trait**](#11226-6-内存管理-trait)
    - [1.1.22.7 7. **所有权和借用 trait**](#11227-7-所有权和借用-trait)
    - [1.1.22.8 8. **迭代器和集合 trait**](#11228-8-迭代器和集合-trait)
    - [1.1.22.9 9. **错误处理 trait**](#11229-9-错误处理-trait)
    - [1.1.22.10 10. **特征对象 trait**](#112210-10-特征对象-trait)
    - [1.1.22.11 11. **同步和并发 trait**](#112211-11-同步和并发-trait)
    - [1.1.22.12 12. **生命周期 trait**](#112212-12-生命周期-trait)
  - [1.1.23 编译器语义](#1123-编译器语义)
<!-- TOC END -->
...
## 1.1.14 14. **Drop** - 定义了当值离开作用域时执行的清理逻辑
