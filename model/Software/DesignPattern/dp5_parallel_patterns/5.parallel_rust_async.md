...
<!-- TOC START -->
- [ ](#1-1-1-1-1-1-1-并行模式parallel-patterns)
  - [1. ](#异步实现)
    - [1.1 ](#1-数据并行模式（data-parallelism）)
    - [1.2 ](#2-任务并行模式（task-parallelism）)
    - [1.3 ](#3-流水线并行模式（pipeline-parallelism）)
    - [1.4 ](#4-工作分割模式（work-decomposition）)
    - [1.5 ](#5-工作窃取模式（work-stealing）)
...
### 1.1.2 2. 任务并行模式（Task Parallelism）
...
### 1.1.4 4. 工作分割模式（Work Decomposition）
...
### 1.1.14 14. 并行数据流模式（Parallel Data Streaming）
